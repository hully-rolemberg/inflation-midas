...
summary(eqdd_1)
aux_eqdd_1_f <- fitted(eqdd_1)

checkresiduals(eqdd_1)
adf.test(residuals(eqdd_1))
accuracy_eqb_1_in <- rmse(diff(inflation_cpi_full)[3:n_train], aux_eqdd_1_f)
...
for (h in 0:(n_test - step)) {
  for (i in 1:28) {
    ...
    aux_eqdd_1_f <- forecast(eqdd_1, newdata = data_test)$mean[step]
    eqdd_1_f <- rbind(eqdd_1_f, aux_eqdd_1_f)
...
accuracy_eqdd_1 <- c(rmse(diff(inflation_cpi_test), eqdd_1_f_avg))
loss_eqdd_1 <- LossLevel(diff(inflation_cpi_test), eqdd_1_f_avg)
